    ...
    if args.which == 'register':
        try:
            ...
            if server_ra.register(args.seed):
                ...
                logger.info('{cmd} listen'.format(cmd=sys.argv[0]), color="WHITE", light=True)
                logger.info('############ IMPORTANT #############', color="YELLOW", light=True)
                logger.info('# You will not be able to connect  #', color="YELLOW", light=True)
                logger.info('# to the web interface until you   #', color="YELLOW", light=True)
                logger.info('# have setup the admin certificate.#', color="YELLOW", light=True)
                logger.info('####################################', color="YELLOW", light=True)
                logger.info('- step 1: Download admin files (admin.*) & CA certificate (ca.crt) from {p}'.format(p=BASE_DIR), color="YELLOW")
                logger.info('- step 2: Generate the admin p12 file:', color="YELLOW")
                logger.info('> openssl pkcs12 -export -out admin.p12 -inkey admin.key -in admin.crt -certfile ca.crt', color="WHITE", light=True)
                logger.info('- step 3: Install admin.p12 and ca.crt into your browser', color="YELLOW")
                logger.info('You should now go check the WEB interface project -> https://github.com/proh4cktive/upki-web')
                logger.info('And the Client project -> https://github.com/proh4cktive/upki-cli')
